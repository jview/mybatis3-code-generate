...
public class MyBatisGeneratorConfigurationParser {
    ...
    private void parseJavaModelGenerator(Context context, Node node) {
        ...
        String targetPackage = attributes.getProperty("targetPackage"); //$NON-NLS-1$
        String targetProject = attributes.getProperty("targetProject"); //$NON-NLS-1$
        String exampleTargetPackage = attributes.getProperty("exampleTargetPackage"); //$NON-NLS-1$
        

        javaModelGeneratorConfiguration.setTargetPackage(targetPackage);
        javaModelGeneratorConfiguration.setTargetProject(targetProject);
        javaModelGeneratorConfiguration.setExampleTargetPackage(exampleTargetPackage);//support examplePackage
    ...
    private String replacePropKey(String propKey, Context context){
    	if(propKey.indexOf("${")>0 && propKey.indexOf("}")>0){
    		String tmp=propKey.substring(propKey.indexOf("${")+2, propKey.indexOf("}"));
    		String v=context.getProperty(tmp);
    		if(v!=null){
    			propKey=propKey.replace("${"+tmp+"}", v);
    		}
    	}
    	return propKey;
    }
